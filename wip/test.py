...
for ref_data in reference_files:
    ...
    # --- Use adaptive threshold for reference images too (consistent pipeline) ---
    ref_gray = cv2.cvtColor(ref_img, cv2.COLOR_BGR2GRAY)
    ref_blur = cv2.GaussianBlur(ref_gray, (5, 5), 0)
    ref_threshold = cv2.adaptiveThreshold(
        ref_blur, 255,
...
# ==========================================
# 4. ORB TEMPLATES (Complex Symbols)
# ==========================================
orb = cv2.ORB_create()
...
while True:
    ...
    # --- Preprocessing: adaptive threshold on grayscale ---
    # Works on local contrast so light surroundings don't wash out shape edges
    ...
    frame_threshold = cv2.adaptiveThreshold(
        ...
        cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
        cv2.THRESH_BINARY_INV,
        blockSize=51,   # increase if large shapes are fragmented
        C=10            # increase if background noise appears
    )

    # Close gaps within shapes
